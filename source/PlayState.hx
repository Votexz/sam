...
class PlayState extends MusicBeatState
{
	...
	public static var ratingStuff:Array<Dynamic> = [
		['F', 0.2], //From 0% to 19%
		['E', 0.41], //From 20% to 39%
		['D', 0.52], //From 40% to 49%
		['C', 0.69], //From 50% to 59%
		['B', 0.8], //From 60% to 68%
		['A', 0.87], //69%
		['AA', 0.9], //From 70% to 79%
		['AAA', 0.95], //From 80% to 89%
	...
	override public function create()
	{
		...
		switch (curStage)
		{
			...
			GameOverSubstate.characterName = 'bf-carry';

				var bg:BGSprite = new BGSprite('flippy/triggered/sky', -600, -225, 0.5, 0.5);
				add(bg);

				var backTrees:BGSprite = new BGSprite('flippy/triggered/backtrees', -625, -200, 0.7, 0.7);
				add(backTrees);

				var trees:BGSprite = new BGSprite('flippy/triggered/trees', -625, -225, 0.8, 0.8);
				add(trees);

				var stageFront:BGSprite = new BGSprite('flippy/triggered/ground', -700, -150, 1.0, 1.0);
				add(stageFront);
				
				boombox = new BGSprite('flippy/speaker', 925, 515, 1, 1, ['boombox']);
				add(boombox);

				fire = new BGSprite('flippy/fire', 500, 360, 1, 1, ['fire idle']);
				add(fire);

				var table:BGSprite = new BGSprite('flippy/triggered/table', -850, -200, 1.3, 1.3);
			...
			GameOverSubstate.characterName = 'bf-carry';
				...
				var bg:BGSprite = new BGSprite('flippy/slaughter/sky', -600, -225, 0.5, 0.5);
				add(bg);

				var backTrees:BGSprite = new BGSprite('flippy/slaughter/backtrees', -625, -200, 0.7, 0.7);
				add(backTrees);

				var trees:BGSprite = new BGSprite('flippy/slaughter/trees', -625, -225, 0.8, 0.8);
				add(trees);

				var stageFront:BGSprite = new BGSprite('flippy/slaughter/ground', -700, -150, 1.0, 1.0);
				add(stageFront);
				...
				var table:BGSprite = new BGSprite('flippy/slaughter/table', -850, -200, 1.3, 1.3);
				table.setGraphicSize(Std.int(table.width * 1.1));
				table.updateHitbox();
				add(table);
				
			case 'htf': //Alt
				var bg:BGSprite = new BGSprite('flippy/alt/sky', -600, -300, 0.1, 0.1);
				add(bg);

				var skyline:BGSprite = new BGSprite('flippy/alt/skyline', -600, -400, 0.5, 0.5);
				add(skyline);

				var stageFront:BGSprite = new BGSprite('flippy/alt/ground', -590, -330, 1.0, 1.0);
				add(stageFront);
				
				var stump:BGSprite = new BGSprite('flippy/alt/stump', 810, 630, 1.0, 1.0);
				add(stump);

			case 'army': //BOnus
				var bg:BGSprite = new BGSprite('flippy/kapow/sky', -600, -300, 0.7, 0.7);
				add(bg);
				
				var wall:BGSprite = new BGSprite('flippy/kapow/bwall', -600, -300, 0.7, 0.7);
				add(wall);

				var scene:BGSprite = new BGSprite('flippy/kapow/scene', -600, -300, 1.0, 1.0);
				add(scene);

				var lights:BGSprite = new BGSprite('flippy/kapow/lights', -800, -200, 1.2, 1.2);
				add(lights);

			case 'treehouse': //BOnus
				var bg:BGSprite = new BGSprite('flippy/cleave/sky', -600, -225, 0.5, 0.5);
				add(bg);
				
				var trees:BGSprite = new BGSprite('flippy/cleave/trees', -625, -225, 0.8, 0.8);
				add(trees);

				var stageFront:BGSprite = new BGSprite('flippy/cleave/ground', -200, -150, 1.0, 1.0);
				add(stageFront);

				lilboi = new BGSprite('flippy/cleave/lilboi', 3200, 890, 1, 1, ['bop']);
				add(lilboi);			
	...
	public function endSong():Void
	{
		...
		if(achievementObj != null) {
			...
			var achieve:String = checkForAchievement(['week1_nomiss', 'week2_nomiss', 'week3_nomiss', 'week4_nomiss',
				...
				'ur_good', 'ur_great', 'hype', 'two_keys', 'toastie', 'debugger']);
